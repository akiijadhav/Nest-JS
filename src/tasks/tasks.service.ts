...
import { FindTaskQueryParams } from '../common/find-task-query.params';
...
export class TasksService {
  ...
  public async findAll(
    filters: FindTaskQueryParams,
    pagination: FindTaskQueryParams,
    userId: string,
  ): Promise<[Task[], number]> {
  ...
  public async addLabels(
    ...
    // 1) Deduplicate labels - DONE
    // 2) Get exiting labels from the database - DONE
    // 3) New labels aren't existing ones - DONE
  ...
  public async removeLabels(
    ...
    // 1) Remove existing labels from the task.labels array
    // 2) Ways to solve
